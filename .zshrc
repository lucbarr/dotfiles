...
# ZSH_THEME_RANDOM_CANDIDATES=(s "robbyrussell" "agnoster" )
...
alias statsu="status"
#uses git add matching with grep
gadd() { git add  $(git status | grep "$@" | sed 's/modified:  //g' | sed 's/deleted:  //g') }
#uses git checkout matching with grep
gcheck() { git checkout  $(git status | grep "$@" | sed 's/modified:  //g' | sed 's/deleted:  //g') }
